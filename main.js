...
function fibonacci(limit) {
  ...
  while (previous1 < limit && previous2 < limit) {
...
// If two players both have a full house, then the three of a kind (within the full house) is
// compared to determine the winner (the highest three of a kind card value wins). See the
// example in the table below.

// If two players both have a two-pair, then the following are compared in order of prece-
// dence to determine the winner: (1) the upper pair value in each hand, (2) the lower pair

// value in each hand, and finally (3) the unpaired card. See the example in the table below.
...
// 5H 5C 6S 7S KD [Pair: 5] 2C 3S 8S 8D TD [Pair: 8] Player 2
// 5D 8C 9S JS AC [High card: A] 2C 5C 7D 8S QH [High card: Q] Player 1
// 2D 9C AS AH AC [Three of a kind: A] 3D 6D 7D TD QD [Flush: Diamonds] Player 2
// 4D 6S 9H QH QC [Pair: Q] 3D 6D 7H QD QS [Pair: Q] Player 1
// 2H 2D 4C 4D 4S [Full House: three 4’s] 3C 3D 3S 9S 9D [Full House: three 3’s] Player 1
// AS 7C 2C 7S AH [two-pair, aces over sevens] TH 8H TC AC 8S [two-pair, tens over eights] Player 1

// The file, poker.txt, contains one-thousand random hands dealt to two players. Each
// line of the file contains ten cards (separated by a single space): the first five are Player
// 1’s cards and the last five are Player 2’s cards. You can assume that all hands are valid
...
function pokerHands(data) {
  ...
  games.forEach(g => {
    ...
    for (let i = 0; i < 5; i++) {
      let flush = true
      let handData = {
        twos: { count: 0, value: 1, cardName: 2 },
        threes: { count: 0, value: 2, cardName: 3 },
        fours: { count: 0, value: 3, cardName: 4 },
        fives: { count: 0, value: 4, cardName: 5 },
        sixes: { count: 0, value: 5, cardName: 6 },
        sevens: { count: 0, value: 6, cardName: 7 },
        eights: { count: 0, value: 7, cardName: 8 },
        nines: { count: 0, value: 8, cardName: 9 },
        tens: { count: 0, value: 9, cardName: 'T' },
        j: { count: 0, value: 10, cardName: 'J' },
        q: { count: 0, value: 11, cardName: 'Q' },
        k: { count: 0, value: 12, cardName: 'K' },
        a: { count: 0, value: 13, cardName: 'A' }
      }
      let found = Object.values(handData).find(o => o.cardName == cards[i][0])
      if (found) {
        found.count++
      }
      console.log(found);
...
let trialData = `TC TS KC 9H 4S 7D 2S 5D 3S AC
AS KD 3D JD 8H 7C 8C 5C QD 6C`
